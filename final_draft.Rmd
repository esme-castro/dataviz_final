...
ggplot(data_viz3, aes(support_mean, mean), color = stabbr) +
  geom_point(aes(color = stabbr)) +
  gghighlight(stabbr %in% c("OR", "MT", "HI"),
...
#RG I like the heatmap idea a lot but this graph is informative as well. It would be interesting to see what Oregon compares to in this graph (so maybe highlight the highest and lowest total revenue states)
...
#RG is this mean percentage of graduation rates? 
```
...
#RG you can remove 225 min policy since there are none that fit
...
#RG these wouldn't run for me error: Error in print(z.1) : object 'z.1' not found
```
