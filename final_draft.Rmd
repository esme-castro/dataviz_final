...
# Very beautiful plot! I love the simplicity of this plot and the OR highlight is very eye-pleasing! 
#Does the dots within state represents school district? Some suggestions: 1. Maybe try get the average graduation rate for each state instead and 2. Divide the revenue by total number of students. This way we can compare each state more objectively. 
...
# Same comment here, does each data point represent district? perhaps calculate the average per state and plot it. 
#maybe label on y-axis can be moved to x-axis ? 
```
...
```{r zviz-final-plot, fig.height=8, fig.width=8}
...
class.z.2010 %>% 
  ...
  theme(panel.grid.major.y = element_blank(), 
        ...
        plot.title.position = "plot",  #left aligned title and subtitle
        plot.title = element_text(family = 'Helvetica', 
                              color = '#666666', 
                              face = 'bold', 
                              size = 18, 
                             ),
        legend.position='bottom',  #moving legend to bottom but needs more wrangling for spacing
